...
@extend_schema(
    ...
    responses={
        ...
        400: {"description": "Bad Request (e.g., invalid data, email exists and is active)"},
...
def signup(request):
    ...
    if user and user.is_active:
        return Response({"error": "이미 가입되어 활성화된 이메일입니다."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    if user:  # Inactive user exists, update their code and password
        ...
        user.save(update_fields=['verification_code', 'code_expiry', 'password', 'is_active'])
    ...
    try:
        ...
        mail_server_url = "http://mail-server:8001/send-verification-email/"
        response = requests.post(mail_server_url, json={"email": email, "code": verification_code}, timeout=15)
...
def verify_email(request):
    ...
    try:
        ...
        return Response({"error": "사용자를 찾을 수 없거나 이미 활성화된 계정입니다."}, status=status.HTTP_404_NOT_FOUND)
